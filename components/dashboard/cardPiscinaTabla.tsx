...
const PhIndicator = ({ value }: { value: number }) => {
  ...
  return (
    <View className="flex-row items-center">
      <View className="h-3 w-3 rounded-full mr-2" style={{ backgroundColor: getColor() }} />
...
const EquipmentItem = ({
  ...
  return (
    <View className="items-center bg-gray-50 p-3 rounded-lg">
      ...
      <View className="mt-1 px-2 py-1 rounded-full" style={{ backgroundColor: getStatusColor() + "20" }}>
        <Text className="text-xs font-geist-semi-bold" style={{ color: getStatusColor() }}>
          {estado}
        </Text>
      </View>
    </View>
  ...
  return (
    <View className="bg-white rounded-xl p-5 shadow-sm border border-gray-200 mb-4">
      ...
      <View className="flex-row justify-between items-center mb-4">
        <Text className="font-geist-semi-bold text-text text-2xl">{pool.nombre}</Text>
        <View className="bg-gray-900 rounded-full px-3 py-1">
          <Text className="font-geist-semi-bold text-white text-xs">{pool.tipo}</Text>
        </View>
      </View>
      ...
      <View className="border-b border-gray-200 pb-4 mb-4">
        <View className="flex-row items-center mb-3">
          ...
          <Text className="text-gray-500 font-geist text-sm mr-2">Propietario:</Text>
          <Text className="text-text font-geist-semi-bold text-sm">{pool.propietario}</Text>
        </View>
      ...
      {/* Equipos */}
      <Text className="text-gray-700 font-geist-semi-bold text-sm mb-3">Equipos:</Text>
      <View className="flex-row flex-wrap justify-between mb-4">
        {pool.equipos.map((equipo: { tipo: string; estado: string; }, index: React.Key | null | undefined) => (
          <View key={index} className="w-[48%] mb-2">
            <EquipmentItem tipo={equipo.tipo} estado={equipo.estado as "Operativo" | "Inactivo" | "Atención urgente" | "Alerta"} />
          </View>
        ))}
      </View>
      ...
      <View className="flex-row justify-between mt-2">
        <Link asChild href={`/(tabs)/${pool.id}`}>
          <Pressable className="bg-gray-900 rounded-lg py-3 flex-1 mr-3 flex-row items-center justify-center">
            ...
            <Text className="text-white font-geist-semi-bold text-sm ml-2">Panel</Text>
          </Pressable>
        </Link>
        <Link asChild href={`/${pool.id}`}>
          <Pressable className="bg-gray-900 rounded-lg py-3 flex-1 flex-row items-center justify-center">
            ...
            <Text className="text-white font-geist-semi-bold text-sm ml-2">Equipos</Text>
          </Pressable>
        </Link>
      </View>
    </View>

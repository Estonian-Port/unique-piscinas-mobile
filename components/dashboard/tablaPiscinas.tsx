...
const TablaPiscinas = () => {
  const datos = Array.from({ length: 20 }, (_, index) => ({
    ...
    equipos: [
      ...
      {
        tipo: 'trasductor',
        estado: index % 5 === 0 ? 'reemplazo' : 'operativo',
      },
      {
        tipo: 'calentador',
        estado: index % 2 === 0 ? 'inactivo' : 'operativo',
      },
    ],
  }));
...
const Encabezado = ({
  ...
  return (
    ...
    >
      <View
        style={{ width: columnWidths.nombre, height: 50 }}
        className="py-4 px-3 items-center justify-center"
      >
        <Text className="font-semibold text-text">Nombre</Text>
      </View>
      <View
        style={{ width: columnWidths.propietario, height: 50 }}
        className="py-4 px-3 items-center justify-center"
      >
        <Text className="font-semibold text-text">Propietario</Text>
      </View>
      <View
        style={{ width: columnWidths.tipo, height: 50 }}
        className="py-4 px-3 items-center justify-center"
      >
        <Text className="font-semibold text-text">Tipo</Text>
      </View>
      <View
        style={{ width: columnWidths.ph, height: 50 }}
        className="py-4 px-3 items-center justify-center"
      >
        <Text className="font-semibold text-text">pH</Text>
      </View>
      <View
        style={{ width: columnWidths.equipos, height: 50 }}
        className="py-4 px-3 items-center justify-center"
      >
        <Text className="font-semibold text-text">Estado Equipos</Text>
      </View>
      <View
        style={{ width: columnWidths.acciones, height: 50 }}
        className="py-4 px-3 items-center justify-center"
      >
...
const Fila = ({
  ...
  return (
    <View className="flex-row bg-white border-b border-gray-200">
      ...
      >
        <View
          className={`p-2 rounded-full ${
            tipo === 'Skimmer' ? 'bg-white border border-gray-300' : 'bg-black'
          }`}
        >
          <Text
            className={`font-medium text-sm ${
              tipo === 'Skimmer' ? 'text-gray-800' : 'text-white'
            }`}
          >
            {tipo}
          </Text>
      ...
      <View
        style={{ width: columnWidths.ph, height: 50 }}
        className="flex-row py-4 px-3 items-center justify-center"
      >
        <View
          className={`w-3 h-3 rounded-full mr-2 ${
            ph < 7 || ph >= 8 ? 'bg-orange-400' : 'bg-green-500'
          }`}
        />

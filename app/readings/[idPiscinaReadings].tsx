...
import { CalendarIcon, ChevronDownIcon, ChevronUpIcon, FilterIcon, SearchIcon } from '@/assets/icons';
  ...
  return (
    <SafeAreaView className="flex-1 bg-gray-50">
      <View className="px-4 pt-2 pb-4">
        <Text className="font-geist-semi-bold text-3xl text-gray-900 mb-1">{pool.name}</Text>
        <Text className="font-geist-semi-bold text-xl text-gray-900 mb-1">Todas las Lecturas</Text>
        <Text className="text-gray-500 text-sm mb-4">Historial completo de mediciones de parámetros del agua</Text>
        ...
        <View className="flex-row mb-4">
          ...
          <Pressable className="bg-white p-3 rounded-lg border border-gray-200 mr-2" onPress={toggleSortDirection}>
            {sortDirection === "desc" ? <ChevronDownIcon size={18} color="#666" /> : <ChevronUpIcon size={18} color="#666" />}
          </Pressable>
        ...
        <View className="flex-row justify-between items-center mb-2">
          <Text className="text-gray-500 text-sm">
            {filteredLecturas.length} {filteredLecturas.length === 1 ? "lectura" : "lecturas"} encontradas
          </Text>
          <Text className="text-gray-500 text-sm">{sortDirection === "desc" ? "Más reciente" : "Más antiguo"}</Text>
        </View>
      </View>
      ...
      <ScrollView className="flex-1 px-4">
        ...
        {sortedLecturas.length === 0 && (
          <View className="items-center justify-center py-10">
            ...
            <Text className="text-gray-500 font-geist text-base mt-2">No se encontraron lecturas</Text>
          </View>
        )}
      </ScrollView>
    </SafeAreaView>
